 ...
 */
export default async function uploadScreenshot(screenshotBuffer: Buffer, testTitle: string): Promise<void> {
  ...
  const { error } = await supabase.storage.from('test-failures').upload(filePath, screenshotBuffer, {
  ...
  const { data: publicUrlData } = supabase.storage.from('test-failures').getPublicUrl(filePath);

  logger.info(`new screenshot of test failure uploaded to: ${publicUrlData.publicUrl}`);
}
