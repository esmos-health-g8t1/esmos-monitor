...
test.describe('Homepage', () => {
  ...
  test('header should have all elements', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: 'Logo of My Website' })).toBeVisible();
    ...
    await expect(page.getByRole('link', { name: 'eCommerce cart' })).toBeVisible();
  ...
  test.describe('main content should have complete elements', () => {
    ...
    test('section 2', async ({ page }) => {
      // section 2
      await expect(
        page.locator('section').filter({ hasText: 'Your Meal, Your Way We offer' }).locator('img')
      ).toBeVisible();
      await expect(page.getByRole('heading', { name: 'Your Meal, Your Way' })).toBeVisible();
      await expect(page.locator('#wrap')).toContainText(
    ...
    test('section 3', async ({ page }) => {
      ...
      await expect(page.getByRole('link', { name: 'Ask us a question' })).toBeVisible();
    });

    test('carousel', async ({ page }) => {
      await expect(page.locator('section').filter({ hasText: 'Your Dynamic Snippet will be' })).toBeVisible();
    });
