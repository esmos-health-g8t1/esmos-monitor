...
test.describe('Services Page', () => {
  ...
  test('should load the services page with correct title and header', async ({ page }) => {
  ...
  test('should display three cards with correct content', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Seasonal Specials' })).toBeVisible();
    await expect(page.locator('#wrap')).toContainText(
    ...
    await expect(page.getByRole('heading', { name: 'Quick Meal Prep' })).toBeVisible();
    await expect(page.locator('#wrap')).toContainText(
  ...
  test('should display happy customers section', async ({ page }) => {
    await expect(page.getByRole('heading', { name: 'Our Happy Customers' })).toBeVisible();
    await expect(page.locator('#wrap')).toContainText('Join our community of food lovers.');
